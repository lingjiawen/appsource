...
export default (onMessage: Function) => {
  ...
  const init = () => {
    socket.onopen = function (_) {
      ...
      isActive = true;
      appDownloadStoreAct.setSocketStatus(true);
      sourceImportStoreAct.setSocketStatus(true);
    };
    ...
    socket.onmessage = function (event) {
      ...
      if (message.event === 'downloadProgress') {
        appDownloadStoreAct.setMessages(message.data)
        return;
      }
      if (message.event === 'sourceImportProgress') {
        sourceImportStoreAct.setMessages(message.data)
        return;
      }
      //更新token
    ...
    socket.onerror = function (_) {
      ...
      reconnect();
      appDownloadStoreAct.setSocketStatus(false);
      sourceImportStoreAct.setSocketStatus(false);
      isActive = false;
    };
