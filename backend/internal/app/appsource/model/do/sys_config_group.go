...
type SysConfigGroup struct {
	gmeta.Meta `orm:"table:sys_config_group, do:true"`
	Id         interface{} `orm:"id,primary" json:"id"`        // 主键
	GroupName  interface{} `orm:"group_name" json:"groupName"` // 配置组名称
	GroupKey   interface{} `orm:"group_key" json:"groupKey"`   // 配置组键名
