...
func GetDomain(ctx context.Context, hasUri ...bool) string {
	...
	// 智能判断Scheme
	scheme := "http"
	if r.TLS != nil || gconv.Bool(r.Header.Get("X-Forwarded-Proto")) {
		scheme = "https"
	}
	// 优先使用自定义头
	if customScheme := r.Header.Get("X-Scheme"); customScheme != "" {
		scheme = customScheme
	}
	...
	if len(hasUri) > 0 && hasUri[0] {
		...
		// 智能截取API前缀
		if pos := gstr.PosI(uri, "/api/"); pos > 0 {
			uri = gstr.SubStr(uri, 0, pos)
		}
		return fmt.Sprintf("%s://%s%s", scheme, host, uri)
	}
